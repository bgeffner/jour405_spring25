...
These differences are key in affecting reporting on likely voter turnout, as it helps identify the number of likely voters — as well as individual voters who vote sporadically and are less likely to vote. While these polls can be helpful, they're not always accurate in depicting eventual voting numbers, thus why it's important to look beyond these numbers when interpreting and predicting likely voter turnout — and accounting for unseen, non-numerical factors that may impact voting results as well. A large majority of rarely/never voters are unsure about voting, thus creating a disparity in predicted voting statistics — if these voters are convinced to vote for a specific candidate, reporting on likely voter turnout might fluctuate far beyond what these differences display.
...
These differences tell me that unsure voters may often be underrepresented in the sample, as a large majority of results depict "always" voter statistics, but fail to display concrete "yes or no" results for rare voter numbers — making it increasingly difficult to predict true voter intentions across the board for unlikely voters.
