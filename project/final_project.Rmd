...
Along with a boxplot, I generated ANOVA, linear regression and chi-square tests (molding formulas based on in-class work with each) to study NBA Team Win Percentage by Rest Splits in the 2024-25 season, with the goal in using this specific dataset being to further confirm if NBA team win percentage relative to various types of rest advantages (in this case, variables included 'no rest,' 'one day's rest' and '2-3 day's rest') are significantly different from my personal expected frequencies noted in the original hypothesis. I initially was having trouble forming the tests (and receiving error messages), but found success after reformatting the data in my original Google Sheet to make it as 'long' a format as possible (more rows than columns) as suggested in feedback from last week's assignment. Through the restructured data, I was able to conclude that, since my chi-square statistic (85) is less than the critical value (112.022), I would accept the null hypothesis and conclude that the observed frequencies are significantly different. There's statistical evidence to show that my distribution of win % data varies and is influenced by more or less rest time before games (that the two variables are interconnected, and not independent of one another) — while my boxplot also separated by rest split category accurately backed up and confirmed my original belief as well, that an increased rest advantage between games generally results in a higher, elevated mean percentage for teams overall (of course, with the potential to be heightened or reduced by outside factors such as strength of schedule, weighted success, etc.).
...
My final test for the team over time findings was a linear regression model analysis, testing whether win percentage could be predicted or influence by a specific year. In this case, I set win percentage as the dependent variable and the year as the independent variable — with hopes that if we know the year for a specific NBA season, the win percentage reliability within it would be reflected/revealed. 
...
total_by_time_period <- read.csv("https://docs.google.com/spreadsheets/d/e/2PACX-1vRK56dBfbzYVb8vYiONb9Sf4H1f4jajM393iHDzw_SXNhgGvXyA_VbU4-bmmlzgU6sWsBe13t49-cnf/pub?output=csv")
