...
### Chi-Square Test for '2024-25 Team Win Percentage by Rest Splits' (UPDATED as suggested without % symbols on 5/3)

```{r}
chisq_result <- chisq.test(current_win_by_split$rest_time, current_win_by_split$win_pct)
chisq_result
```
```{r}
alpha <- 0.05
df <- length(current_win_by_split$team) - 1
